...
export default function AdminCourseEnrollmentModal({ enrollment, onClose, onSaved }) {
  ...
  useEffect(() => {
    if (enrollment) {
      ...
      supabase
        .from('payments')
        .select('*')
        .eq('lead_course_id', enrollment.id)
  ...
  const handleSave = async () => {
    setSaving(true);
    const { error } = await supabase.from('leads_course').update(form).eq('id', enrollment.id);
    setSaving(false);
    if (error) {
  ...
  return (
    <Modal isOpen={Boolean(enrollment)} onClose={onClose} title="Manage Enrollment">
      <div className="grid sm:grid-cols-2 gap-4 mb-6 pb-6 border-b-2 border-secondary/10">
        ...
        <InfoRow label="Course" value={enrollment.course_title} />
