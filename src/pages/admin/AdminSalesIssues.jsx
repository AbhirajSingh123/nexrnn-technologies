...
export default function AdminSalesIssues() {
  ...
  const load = useCallback(async () => {
    ...
    if (error) toast.error('Failed to load sales issues.');
    setRows(data ?? []);
    setLoading(false);
  }, []);
  ...
  const filtered = rows.filter((r) => {
    ...
    if (q && ![r.issue_id, r.sales_id, r.name, r.mobile, r.email, r.issue].join(' ').toLowerCase().includes(q)) return false;
    return true;
  });
  ...
  const columns = [
    ...
    { key: 'sales_id', label: 'Sales ID', render: (r) => <span className="font-mono text-xs">{r.sales_id}</span> },
  ...
  return (
    <div>
      <h1 className="font-heading text-3xl text-secondary mb-1">Sales Team Issues</h1>
      <p className="text-sm text-muted normal-case mb-6">Issues reported by sales team members — respond and update the status.</p>

      <AdminFilterBar
        search={search}
        onSearchChange={setSearch}
        searchPlaceholder="Search issue ID, sales ID, name…"
        extra={
          <div>
      ...
      <div className="mb-4">
        <ExportButtons rows={exportRows} columns={columns} filename="sales-issues" title="Sales Team Issues" excludeKeys={['view']} />
      </div>
            ...
            <div className="p-5 sm:p-7 pt-4 space-y-5">
              <div className="bg-accent border-2 border-secondary/15 px-4 py-3.5 text-sm">
                ...
                <div className="grid grid-cols-2 gap-y-2 gap-x-3">
                  ...
                  <p className="font-mono text-xs text-secondary">{detail.sales_id}</p>
                  <p className="text-muted text-xs">Mobile</p>
                  <p className="text-secondary">{detail.mobile}</p>
              ...
              <div className="border-2 border-secondary/15 px-4 py-3.5 text-sm">
                <p className="text-[10px] font-bold uppercase tracking-wide text-muted mb-1.5">Issue</p>
                <p className="text-secondary normal-case whitespace-pre-wrap">{detail.issue}</p>
              </div>
              ...
              {detail.attachment_path && (
                ...
                >
                  {downloading ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />} Open Attachment
                </button>
              )}
              ...
              <div>
                <label className={labelClass}>Response (visible to the member)</label>
                <textarea
                  rows={3}
