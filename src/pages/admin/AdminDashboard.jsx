...
import { Inbox, Briefcase, GraduationCap, Layers, BookOpen, Search, CalendarDays, IndianRupee, Wallet, Newspaper, MonitorPlay, Users } from 'lucide-react';
import { supabase } from '@/services/supabaseClient';
import { ADMIN_ROUTES } from '@/constants/adminRoutes';
import { formatDateTimeWithDay } from '@/utils/formatDateTime';
import { fetchAnalyticsEvents, countByEvent, countUniqueVisitors } from '@/data/analyticsRepo';
...
const TYPE_META = {
  ...
  workshop: { label: 'Workshop', to: ADMIN_ROUTES.leadsWorkshops },
};
...
export default function AdminDashboard() {
  ...
  const [paymentsInfo, setPaymentsInfo] = useState({ total: 0, today: 0 });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [contactRes, serviceRes, courseRes, workshopRes] = await Promise.all([
        supabase.from('leads_contact').select('*').order('created_at', { ascending: false }).limit(50),
        supabase.from('leads_service').select('*').order('created_at', { ascending: false }).limit(50),
        supabase.from('leads_course').select('*').order('created_at', { ascending: false }).limit(50),
        supabase.from('leads_workshop').select('*').order('created_at', { ascending: false }).limit(50),
      ]);
      ...
      const combined = [
        ...
        ...(workshopRes.data ?? []).map((r) => ({ ...r, type: 'workshop', label: r.workshop_title })),
      ].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));

      setRecent(combined.slice(0, 60));
      ...
      try {
        ...
        setUniqueUsers(countUniqueVisitors(events));
      } catch {
        setTraffic(null);
      }

      // Payments received (total + today)
      try {
        const { data: pays } = await supabase.from('payments').select('amount, status, created_at');
        const paid = (pays ?? []).filter((r) => r.status === 'paid');
        const todayStr = new Date().toDateString();
        const total = paid.reduce((s, r) => s + (Number(r.amount) || 0), 0);
        const today = paid
          .filter((r) => new Date(r.created_at).toDateString() === todayStr)
          .reduce((s, r) => s + (Number(r.amount) || 0), 0);
        setPaymentsInfo({ total, today });
      } catch {
        setPaymentsInfo({ total: 0, today: 0 });
      }
    };
    load();
  }, []);
  ...
  const { visibleItems, hasMore, loadMore, total, shown } = useLoadMore(
    filteredRecent,
    `${typeFilter}|${search}|${filteredRecent.length}`,
    10
  );
  ...
  return (
    <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
        ...
        <StatCard icon={CalendarDays} label="Workshop Enrollments" value={counts.leads_workshop} to={ADMIN_ROUTES.leadsWorkshops} />
        ...
        <StatCard icon={MonitorPlay} label="Workshops Listed" value={counts.workshops} to={ADMIN_ROUTES.workshops} />
