...
export default function AdminAnalytics() {
  ...
  const counts = useMemo(() => countByEvent(events), [events]);
  const chart = useMemo(() => dailyPageViews(events, 14), [events]);
  const blogs = useMemo(() => topBlogPosts(events, 8), [events]);
  const pages = useMemo(() => topPages(events, 8), [events]);
        ...
        <>
          ...
          <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
            <h2 className="text-xl text-secondary normal-case">
              Recent Events <span className="text-sm text-muted">({events.length})</span>
            </h2>
            <ExportButtons
              rows={exportRows}
