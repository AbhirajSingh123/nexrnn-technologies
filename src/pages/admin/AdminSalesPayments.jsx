...
import { fetchSalesWithdrawals, updateSalesWithdrawal } from '@/data/salesWithdrawalsRepo';
...
/** Payment ki poori info ke saath mailto link (member ki mail id par) */
function buildPaymentMailto(r) {
  ...
  return `mailto:${encodeURIComponent(r.salesEmail || '')}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}

export default function AdminSalesPayments() {
      ...
      await downloadWithdrawalSlipPDF(
        {
          ...
          name: detail.name,
          mentorId: detail.salesId, // slip helper ka generic id field
          idLabel: 'Sales ID',
        ...
        { email: detail.salesEmail, phone: detail.salesPhone }
      );
    } catch {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchSalesWithdrawals();
      setRows(data ?? []);
    } catch {
  ...
  const filtered = baseRows.filter((r) => {
    ...
    if (!q) return true;
    return [r.withdrawalCode, r.salesId, r.name, r.salesEmail, r.salesPhone, r.refNo, r.upiId, r.accNo]
      .join(' ')
      .toLowerCase()
      .includes(q);
  });
  ...
  const columns = [
    ...
    { key: 'sales_id', label: 'Sales ID', render: (r) => <span className="font-mono text-xs">{r.salesId}</span> },
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateSalesWithdrawal(detail.id, {
        status: detail.status,
        refNo: detail.refNo,
        adminMessage: detail.adminMessage,
        processedAt,
  ...
  return (
    <div>
      <h1 className="font-heading text-3xl text-secondary mb-1">Sales Team Payments</h1>
      <p className="text-sm text-muted normal-case mb-6">
        Withdrawal requests from the sales team — accept, track reference numbers and mark payments done.
      </p>
      ...
      <AdminFilterBar
        ...
        searchPlaceholder="Search payment ID, sales ID, name, ref no…"
      />
      ...
      <div className="mb-4">
        <ExportButtons
          rows={exportRows}
          columns={columns}
          filename={tab === 'requested' ? 'sales-requested-payments' : 'sales-payment-history'}
        ...
        >
          <div className="w-full max-w-lg bg-white border-2 border-secondary max-h-[88vh] overflow-y-auto">
            ...
            <div className="p-5 sm:p-7 pt-4 space-y-5">
              {/* Member details (auto-fetched) */}
              <div className="bg-accent border-2 border-secondary/15 px-4 py-3.5 text-sm">
                <p className="text-[10px] font-bold uppercase tracking-wide text-muted mb-2">Member Details</p>
                <div className="grid grid-cols-2 gap-y-2 gap-x-3">
                  <p className="text-muted text-xs">Name</p>
                  <p className="text-secondary font-semibold">{detail.name}</p>
                  <p className="text-muted text-xs">Sales ID</p>
                  <p className="font-mono text-xs text-secondary">{detail.salesId}</p>
                  <p className="text-muted text-xs">Email</p>
                  <p className="text-secondary break-all">{detail.salesEmail || '—'}</p>
                  <p className="text-muted text-xs">Phone</p>
                  <p className="text-secondary">{detail.salesPhone || '—'}</p>
              ...
              <div>
                <label className={labelClass}>Message (visible to the member)</label>
                <textarea
                  rows={3}
