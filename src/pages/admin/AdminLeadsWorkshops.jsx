...
export default function AdminLeadsWorkshops() {
  ...
  const { visibleItems, hasMore, loadMore, total, shown } = useLoadMore(
    filteredRows,
    `${search}|${dateFrom}|${dateTo}|${workshopFilter}|${statusFilter}`
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (r) => (
        <a
          href={buildEnrollmentMailto({ name: r.name, email: r.email, batchId: r.batch_id, referenceId: r.reference_id, itemTitle: r.workshop_title, type: 'workshop' })}
          className="text-primary font-semibold hover:underline text-xs"
          title={`Email ${r.email}`}
        >
          Send
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        ...
        <ExportButtons rows={rows} columns={columns} filename="workshop-registrations" title="Workshop Registrations" excludeKeys={['certificate', 'mail', 'manage']} />
