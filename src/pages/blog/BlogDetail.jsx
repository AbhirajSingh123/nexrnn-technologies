...
import { incrementBlogViews } from '@/data/blogRepo';
import { trackBlogRead, trackCtaClick } from '@/utils/analytics';
...
export default function BlogDetail() {
  ...
  // Article submission email (Write for Us) - pre-filled template ke saath
  const articleSubmitMailto = `mailto:nexrnntechnologies@gmail.com?subject=${encodeURIComponent(
    'Article Submission - [Your Article Title]'
  )}&body=${encodeURIComponent(
    'Hello NexRNN Team,\n\n' +
      'I would like to publish my article on your blog. Here are my details:\n\n' +
      'Author Name: \n' +
      'Mobile No: \n' +
      'Author Bio (About the Author paragraph): \n\n' +
      'Article Title: \n' +
      'Article Content: (Please find attached - WORD/PDF)\n\n' +
      'Thank you.\n'
  )}`;
    ...
    <>
      ...
      <div className="bg-white py-12">
        <div className="container-section max-w-3xl mx-auto">
          ...
          {post.ctaUrl && (
            <div className="card-base mt-10 p-6 sm:p-8 bg-secondary text-white border-b-4 border-primary">
              ...
              <h3 className="text-lg sm:text-xl font-heading text-white mt-2 mb-4 normal-case">
                Found this article helpful? Take the next step &mdash;
              </h3>
              {post.ctaUrl.startsWith('/') ? (
                <Link
                  to={post.ctaUrl}
                  onClick={() => trackCtaClick(post.ctaText || 'Learn More', post.ctaUrl)}
          ...
          {/* Write for Us - guest authors can submit their article via email */}
          <div className="card-base mt-12 p-6 sm:p-8 bg-accent border-2 border-dashed border-secondary/30">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
              <div className="flex-1">
                <span className="inline-flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest text-primary mb-2">
                  <PenLine size={13} /> Write for Us
                </span>
                <h3 className="text-lg sm:text-xl font-heading text-secondary mb-2 normal-case">
                  Want to Publish Your Article on Our Blog?
                </h3>
                <p className="text-sm text-muted normal-case leading-relaxed mb-3 max-w-xl">
                  Share your knowledge with thousands of readers! No matter which field
                  or background you come from — students, professionals, business owners,
                  or enthusiasts — if you have something valuable to share, we would love
                  to feature your article on our blog with full author credit and your bio.
                </p>
              </div>
