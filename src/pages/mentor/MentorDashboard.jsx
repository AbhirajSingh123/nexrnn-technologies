...
import { IndianRupee, Wallet, CalendarDays, Users, Layers, PartyPopper, Percent, Loader2 } from 'lucide-react';
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted normal-case mb-6">
        Welcome back{mentor?.name ? `, ${mentor.name}` : ''} — here is your mentorship overview.
      </p>
      ...
      {loading || !data ? (
        ...
        <>
          ...
          {(data.wallet?.pending ?? 0) > 0 && (
            <div className="card-base bg-orange-50 border-2 border-orange-200 p-4 mb-8 text-sm text-orange-700 normal-case">
              <b>{inr(data.wallet.pending)}</b> withdrawal request is pending with the admin (Created / In progress).
            </div>
          )}

          {data.assignedCourses === 0 && data.assignedWorkshops === 0 && (
            <div className="card-base bg-white p-8 mt-8 text-center">
              <Loader2 size={0} />
              <p className="text-sm text-muted normal-case">
                No courses or workshops are assigned to you yet. The NexRNN team will assign your programs soon —
                earnings will appear here automatically.
              </p>
            </div>
          )}
        </>
