...
export default function SalesAnnouncements() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const d = await salesData('announcements');
      setRows(d.rows || []);
    } catch (err) {
      if (err?.status === 401) {
  ...
  const handleReact = async (a, emoji) => {
    ...
    try {
      const d = await salesData('announcement_react', { announcement_id: a.id, emoji });
  ...
  const handleReply = async (a, text, done) => {
    ...
    setBusyKey(key);
    try {
      const d = await salesData('announcement_reply', { announcement_id: a.id, message });
